...
#|
  Процедура g возвращает 0 для 0, либо 2^n.
  g(n) = 2^n | n > 0,
  g(0) = 0
...
#|
  ...
  (A 1 (A 1 (A 2 1)) =>
  (A 1 (A 1 2)) => ...
  Здесь мы можем воспользоваться пониманием, как работает
  функция g(n) и подставить результат её работы.
  ... => (A 1 4) =>
  2^{2^{2}}
