...
#|
  ...
  а. Напишите процедуру pseudoremainder-terms, которая работает в точности как remainder-terms, но только
  прежде, чем позвать div-terms, домножает делимое на множитель целости, описанный выше. Модифицируйте
  gcd-terms так, чтобы она использовала pseudoremainder-terms, и убедитесь, что теперь greatest-common-divisor
  выдает ответ с целыми коэффициентами в примере из упражнения 2.95

  б. Теперь у НОД целые коэффициенты, но они больше, чем коэффициенты P1. Измените gcd-terms, чтобы она
  убирала общий множитель из коэффициентов ответа путем деления всех коэффициентов на их (целочисленный)
  НОД.
|#
...
(define (install-real-package)
  ...
  (put 'gcd '(real real)
    (lambda (x y)
      (let ((int-x (shift-to x 'integer))
            (int-y (shift-to y 'integer)))
        (if (and int-x int-y)
            (greatest-common-divisor int-x int-y)
            (make-integer 1)))))
...
(define (install-polynomial-package)
  ...
  (define (pseudoremainder-terms P Q)
    (define (integerizing-factor P Q)
      (let ((p-head (first-term P))
            (q-head (first-term Q)))
        (let ((order-p-head (order p-head))
              (order-q-head (order q-head)))
          (make-term 0
                     (expt (coeff q-head)
                           (+ 1 (- order-p-head order-q-head)))))))

    (cadr (div-terms (mul-term-by-all-terms (integerizing-factor P Q)
                                            P)
                     Q)))
  ...
  (define (gcd-terms a b)
    (define (coeff-gcd TL)
      ...
      (list (make-term 0 (rec TL))))

    (define (rec a b)
      (if (empty-termlist? b)
          a
          (rec b (pseudoremainder-terms a b))))

    (let ((gc-divisor (rec a b)))
      (car (div-terms gc-divisor
                      (coeff-gcd gc-divisor)))))
...
(define pol1 (make-polynomial 'x '((4 1) (3 -1) (2 -2) (1 2))))
(define pol2 (make-polynomial 'x '((3 1) (1 -1))))

(define gcd-pol1-pol2 (make-polynomial 'x '((2 -1) (1 1))))
(define pol1/gcd (make-polynomial 'x '((2 -1) (0 2))))
(define pol2/gcd (make-polynomial 'x '((1 -1) (0 -1))))

(check-true (equ? (greatest-common-divisor pol1 pol2) gcd-pol1-pol2))
(check-true (equ? (car (div pol1 gcd-pol1-pol2)) pol1/gcd))
(check-true (equ? (car (div pol2 gcd-pol1-pol2)) pol2/gcd))
...
(check-true (equ? (greatest-common-divisor Q1 Q2) P1))
